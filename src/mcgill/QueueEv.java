...
public class QueueEv {
   ...
   class Arrival extends Event {
      public void actions() {
         ...
         /* Here, make servList = Fog capacity at a time. FC = fog.cpu - task.cpu. 
          * IF fog.cpu <= 0, add the task to the queue 
          * However, check the pool to see if the pool can attend to it */
   ...
   class Departure extends Event {
      public void actions() {
    	  // When a customer departs, return the fog.cpu, so, fog.cpu = task.cpu + fog.cpu
    	  // cust = servList.removeFirst(); fog.cpu += task.cpu
